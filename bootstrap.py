...
path_schema = And(str_schema, And(lambda x: os.path.isfile(x), error="file not found: {}"))
patch_schema = And([str], And(lambda x: len(x) == len(set(x)), error="duplicates not allowed: {}"))
...
config_schema = Schema({
    ...
    "controlplane": {
        ...
        Optional("patches"): patch_schema,
        "nodes": {
            str_schema: Or(patch_schema, None)
        }
    },
    "worker": {
        Optional("patches"): patch_schema,
...
parser = argparse.ArgumentParser(description="Bootstrap a provisioned Talos Linux cluster.")
parser.add_argument("config", metavar="<config-file>", help="bootstrap configuration of the cluster")
...
def command(name):
    ...
    return lambda *a: [print("==>", ' '.join([binary, *a])), subprocess.run([binary, *a])]
...
talosctl("gen", "config", config['cluster']['name'],
         f"https://{config['controlplane']['record']}.{config['cluster']['domain']}:6443",
         "--with-cluster-discovery=false", "--with-secrets", config['cluster']['secrets'],
...
worker_nodes = [with_domain(n) for n in config['worker']['nodes'].keys()]
...
for (node, node_patches) in config['controlplane']['nodes'].items():
    node_fqdn = with_domain(node)
    talosctl("apply-config", "--insecure", "--nodes", node, "--file", "controlplane.yaml",
             *[e for p in [*controlplane_patches, *(node_patches or [])] for e in ("--config-patch", p)])
...
for (node, node_patches) in config['worker']['nodes'].items():
    ...
    talosctl("apply-config", "--insecure", "--nodes", node, "--file", "worker.yaml",
             *[e for p in [*worker_patches, *(node_patches or [])] for e in ("--config-patch", p)])
...
helm("install", "cilium", "cilium/cilium", "--namespace", "kube-system",
     "--timeout", timeout, "--wait", *[e for o in cilium_opts for e in ("--set", o)])
...
if 'sops' in config['cluster']:
    ...
    kubectl("create", "secret", "generic", "sops-gpg",
            "--namespace=flux-system", f"--from-file={config['cluster']['sops']}")
