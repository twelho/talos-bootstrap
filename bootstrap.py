...
def main():
    ...
    if config["cluster"]["cilium"].get("gateway-api", {}).get("enabled"):
        ...
        # and it registers no reconciliation actions for HTTPRoutes due to code not handling missing CRD schema.
        ...
        # TODO: Send a upstream Cilium patch.
        kubectl(
            "apply",
            "-f",
    ...
    # Wait for Cilium CRDs to be registered (needed if manifests use CiliumGatewayClassConfig etc.)
    if config["cluster"]["cilium"].get("gateway-api", {}).get("enabled"):
